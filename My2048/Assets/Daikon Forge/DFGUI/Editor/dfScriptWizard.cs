...
public class dfScriptWizard : EditorWindow
{
	...
	private void saveAndAttachScript()
	{
		...
		//InternalEditorUtility.AddScriptComponentUncheckedUndoable( gameObj, script );
		var oldScript = gameObj.GetComponent<MonoScript>();
		if (oldScript != null)
		{
			Destroy(oldScript);
		}
		...
		{
			gameObj.AddComponent(script.GetClass());
		}
